<template>
    ...
    <div v-if="loading" class="flex justify-center items-center">
      <UIcon name="i-svg-spinners:3-dots-fade" dynamic class="text-6xl"/></div>
            ...
            <li v-for="repository in repos" :key="repository.id" class="border border-gray-200 dark:border-gray-800 rounded-sm p-4 hover:bg-gray-200 dark:hover:bg-gray-800 font-mono">
              <a :href="repository.html_url" target="_blank">
                <div class="flex items-center justify-between text-sm">
                    <div class="font-semibold capitalize">{{ repository.name }}</div>
                    <div class="justify-center">
                      {{ repository.stargazers_count }} 
                      <UIcon name="i-heroicons-star-20-solid" dynamic class="text-xl"/>
                    </div>
                </div>
                <p class="text-sm">
